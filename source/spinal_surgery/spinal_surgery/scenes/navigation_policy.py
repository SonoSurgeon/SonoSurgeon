...
def run(
    ...
    def update_us_command_from_probe_policy(obs_img: torch.Tensor):
        ...
        # IMPORTANT: same as first script.
        # Do NOT use US_slicer.update_cmd(cmd), because that may apply different internal semantics.
        next_cmd = US_slicer.current_x_z_x_angle_cmd + cmd
        next_cmd = torch.maximum(next_cmd, init_cmd_pose_min)
        next_cmd = torch.minimum(next_cmd, init_cmd_pose_max)
        US_slicer.current_x_z_x_angle_cmd = next_cmd
    ...
    # -------------------------------------------------------------------------
    # Main loop: fully synchronous, no decimation
    # -------------------------------------------------------------------------
    while simulation_app.is_running():
        ...
        if step_i > 0:
            ...
            world_to_ee_target_pos, world_to_ee_target_quat_us, raw_policy_action, policy_action = (
            ...
            if step_i % 30 == 0:
                print(
                    f"[STEP {step_i}] IK={ik_mode} obs={tuple(obs_img.shape)} "
                    f"min/max={obs_img.min().item():.4f}/{obs_img.max().item():.4f}",
                    flush=True,
                )
                print(
                    "[STEP] raw action:",
                    raw_policy_action.detach().cpu().numpy(),
                    flush=True,
                )
                print(
                    "[STEP] scaled action:",
                ...
                print(
                    "[STEP] slicer cmd:",
                    US_slicer.current_x_z_x_angle_cmd.detach().cpu().numpy(),
                    flush=True,
                )
