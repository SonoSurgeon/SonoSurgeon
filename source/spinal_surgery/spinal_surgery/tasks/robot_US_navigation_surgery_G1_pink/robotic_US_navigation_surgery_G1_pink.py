...
scene_cfg = YAML().load(
    open(
        f"{PACKAGE_DIR}/tasks/robot_US_navigation_surgery_G1_pink/cfgs/robotic_US_navigation_surgery_G1_pink.yaml",
        "r",
    )
)
...
# Viewer camera
CAMERA_EYE = [-0.832, 0.279, 1.631]
CAMERA_TARGET = (-0.121, -0.318, 1.260)
...
class roboticUSNavigationSurgeryCfg(DirectRLEnvCfg):
    # env
    decimation = 2
    episode_length_s = robot_pose_cfg["time_length"][_run_mode]
    action_scale = 1
    action_space = 5
...
class roboticUSNavigationSurgeryEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        robot_rand_pos_cfg = robot_rand_cfg.get(
            "position",
        ...
        self.probe_cmd_pose_trajs = []  # list of (N_envs, 3) tensors: [x_cmd, z_cmd, angle_cmd]
    ...
    def _log_probe_cmd_pose(self):
        ...
        ee_pose_w_robot = self.robot.data.body_state_w[:, self.US_ee_jacobi_idx, 0:7]
        ...
        self.probe_cmd_pose_trajs.append(self.cur_cmd_pose.detach().cpu())

    def _sample_random_probe_init_cmd(self, env_ids: Sequence[int] | torch.Tensor | None = None) -> None:
        """Sample a random probe command on the back inside the init range."""
        if env_ids is None:
            env_ids_t = torch.arange(self.scene.num_envs, device=self.sim.device, dtype=torch.long)
        else:
        ...
        self.US_slicer.current_x_z_x_angle_cmd[env_ids_t] = rand_cmd
    ...
    def _update_us_command_from_left_policy(self, probe_obs_img: torch.Tensor) -> None:
        """Update the probe surface command online from the navigation policy."""
        if not hasattr(self, "left_policy") or self.left_policy is None:
            return

        probe_policy = self.left_policy
        with torch.no_grad():
            policy_action = probe_policy(probe_obs_img.to(self.sim.device))

        policy_action = torch.clamp(
            policy_action * torch.tensor([0.5, 0.5, 0.04], device=self.sim.device),
            min=-torch.tensor([1.0, 1.0, 0.1], device=self.sim.device),
            max=torch.tensor([1.0, 1.0, 0.1], device=self.sim.device),
        )
        ...
        # Current EE pose in WORLD, expressed in the US convention.
        ee_pose_w_robot = self.robot.data.body_state_w[:, self.US_ee_jacobi_idx, 0:7]
        ee_pos_w = ee_pose_w_robot[:, 0:3]
        ee_quat_w = ee_pose_w_robot[:, 3:7]
        ee_rotmat_w = matrix_from_quat(ee_quat_w)
        ...
        cmd = torch.cat([dx_dz_human[:, [0, 2]], policy_action[:, 2:3]], dim=-1)

        next_cmd = self.US_slicer.current_x_z_x_angle_cmd + cmd
        next_cmd = torch.maximum(next_cmd, self.init_cmd_pose_min)
        next_cmd = torch.minimum(next_cmd, self.init_cmd_pose_max)
        self.US_slicer.current_x_z_x_angle_cmd = next_cmd
    ...
    def _apply_us_command(self):
        ...
        # Aggiorna la probe policy una sola volta per observation/RL step.
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        r_allowed = self.vertebra_viewer.traj_radius + 0.001
    ...
    def position_probe(self, num_steps: int = 250):
        """
        Run the frozen probe policy to position the US probe.

        num_steps is the number of policy/RL steps, not raw physics steps.
        For each policy step:
        - compute one fresh US observation;
        - update the frozen probe policy once;
        - hold the resulting US target for cfg.decimation physics substeps;
        - keep the drill wrist frozen at its initial pose.
        """
        is_rendering = self.sim.has_gui() or self.sim.has_rtx_sensors()
        dt = float(self.physics_dt)

        # ------------------------------------------------------------------
        # 1) Save initial drill pose in BASE frame and keep it fixed.
        # ------------------------------------------------------------------
        self.get_drill_ee_pose_b()

        drill_hold_pos_b = self.drill_ee_pos_b.detach().clone()
        drill_hold_quat_b = self.drill_ee_quat_b.detach().clone()

        self._drill_target_pos_b = drill_hold_pos_b.clone()
        self._drill_target_quat_b = drill_hold_quat_b.clone()

        # ------------------------------------------------------------------
        # 2) Use full IK joint set: waist/torso + US arm + drill arm.
        # ------------------------------------------------------------------
        joint_ids_t = self.full_joint_ids
        n_ctrl = len(self.full_joint_ids_list)
        N = self.scene.num_envs

        for _ in range(num_steps):

            # ==============================================================
            # POLICY STEP: one observation + one frozen-probe-policy update.
            # ==============================================================

            # Refresh human/world state.
            self.human_world_poses = self.human.data.body_link_state_w[:, 0, 0:7]
            self.world_to_human_pos = self.human_world_poses[:, 0:3]
            self.world_to_human_rot = self.human_world_poses[:, 3:7]
            self.world_to_base_pose = self.robot.data.root_link_state_w[:, 0:7]

            # Compute latest US image. This increments self.num_step.
            self._get_observations()

            # Optional, only needed if you want reset-time diagnostics during positioning.
            self.get_traj_to_tip_state()

            # Update US target once from frozen guidance policy.
            # _apply_us_command() is guarded by last_probe_policy_update_step,
            # so it cannot apply the same policy step twice.
            self._apply_us_command()

            # Keep drill target frozen throughout probe positioning.
            self._drill_target_pos_b = drill_hold_pos_b
            self._drill_target_quat_b = drill_hold_quat_b

            # ==============================================================
            # PHYSICS SUBSTEPS: hold same US/drill targets for decimation.
            # ==============================================================

            for _sub in range(self.cfg.decimation):
                self._sim_step_counter += 1

                # Refresh current EE/base poses before solving IK.
                self.get_US_ee_pose_b()
                self.get_drill_ee_pose_b()
                self.world_to_base_pose = self.robot.data.root_link_state_w[:, 0:7]

                # Targets are already expressed in BASE frame.
                us_t_np = self._us_target_pos_b.detach().cpu().numpy().astype(
                    np.float64,
                    copy=False,
                )
                us_R_np = matrix_from_quat(self._us_target_quat_b).detach().cpu().numpy().astype(
                    np.float64,
                    copy=False,
                )

                dr_t_np = self._drill_target_pos_b.detach().cpu().numpy().astype(
                    np.float64,
                    copy=False,
                )
                dr_R_np = matrix_from_quat(self._drill_target_quat_b).detach().cpu().numpy().astype(
                    np.float64,
                    copy=False,
                )

                q_all_np = self.robot.data.joint_pos.detach().cpu().numpy()
                q_next_ctrl_np = np.zeros((N, n_ctrl), dtype=np.float32)
                ...
                for i in range(N):
                    ...
                    # Moving target: US probe.
                    self.pink_us_task[i].set_target(
                        pin.SE3(us_R_np[i], us_t_np[i])
                    )

                    # Frozen target: drill wrist.
                    self.pink_drill_task[i].set_target(
                        pin.SE3(dr_R_np[i], dr_t_np[i])
                    )
                    ...
                    q_next_pin = self.pink_cfgs[i].q
                    q_next_ctrl_np[i, :] = q_next_pin[self._pin_qidx_full_ctrl].astype(
                        np.float32,
                        copy=False,
                    )
                ...
                q_next_ctrl_t = torch.clamp(
                    q_next_ctrl_t,
                    joint_min.unsqueeze(0) + safety_margin,
                    joint_max.unsqueeze(0) - safety_margin,
                )
            ...
            # Log once per policy step, not once per raw physics step.
            if scene_cfg.get("if_record_traj", False):
                self._log_probe_cmd_pose()
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if scene_cfg["if_record_traj"]:
            ...
            if hasattr(self, "probe_cmd_pose_trajs") and len(self.probe_cmd_pose_trajs) > 0:
                probe_cmd_pose_tensor = torch.stack(self.probe_cmd_pose_trajs, dim=1)  # (N,T,3)
                torch.save(probe_cmd_pose_tensor, record_path + "probe_cmd_pose.pt")

            self.probe_cmd_pose_trajs = []
        ...
        if hasattr(self, "last_tip_pos_along_traj"):
            vertebra_depth = 0.0328  # 32.8 mm

            penetration_pct = (
                (self.total_insertion - self.safe_height) / vertebra_depth
            ).clamp(0.0, 1.0) * 100.0

        if hasattr(
            self, "total_rewards"
        ) & hasattr(
            self, "total_insertion"
        ) & hasattr(
            self, "last_traj_to_tip_sin"
        ) & hasattr(
            self, "last_tip_to_traj_dist"
        ) & hasattr(
            self, "angle"
        ):
            wandb.log(
                {
                    ...
                    "penetration_pct": penetration_pct.mean().item(),
                }
            )
