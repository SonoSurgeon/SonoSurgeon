...
def run(
    ...
    print(f"[CONFIG] robot_type={robot_type} IK={ik_mode} sim_dt={sim.get_physics_dt():.6f} "
          f"control_decimation={args_cli.control_decimation} obs_scale={obs_scale}", flush=True)
    ...
    def update_us_command_from_probe_policy(obs_img: torch.Tensor):
        ...
        prev_cmd = US_slicer.current_x_z_x_angle_cmd.clone()
        unclamped_cmd = prev_cmd + cmd
        next_cmd = torch.maximum(unclamped_cmd, init_cmd_pose_min)
        next_cmd = torch.minimum(next_cmd, init_cmd_pose_max)
        clamp_mask = (next_cmd != unclamped_cmd)
